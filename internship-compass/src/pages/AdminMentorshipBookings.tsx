...
const AdminMentorshipBookings = () => {
  ...
  // Updated Load More to trigger your specific full-screen loader
  const handleLoadMore = async () => {
    setLoading(true);
    // Mimic the load delay you have in the initial fetch
      ...
      <div className="relative z-10 flex-grow">
        ...
        <section className="pt-24 lg:pt-32 pb-12 gradient-hero">
          <div className="container mx-auto px-4 lg:px-8">
            ...
            >
              <motion.h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-primary-foreground mb-5">
                Mentorship{" "}
        ...
        <div className="container mx-auto px-4 lg:px-8 py-10 flex flex-col items-center">
          ...
          {/* Quick Stats Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12 w-full max-w-5xl">
            <Card className="bg-card/80 backdrop-blur-sm border-accent/20 hover:border-accent/40 transition-all">
              <CardContent className="pt-6 text-center">
                ...
                <div className="mt-2 text-sm text-foreground/60">Total Bookings</div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-card/80 backdrop-blur-sm border-accent/20 hover:border-accent/40 transition-all">
              <CardContent className="pt-6 text-center">
                ...
                <div className="mt-2 text-sm text-foreground/60">Upcoming Sessions</div>
              </CardContent>
            </Card>

            <Card className="bg-card/80 backdrop-blur-sm border-accent/20 hover:border-accent/40 transition-all">
              <CardContent className="pt-6 text-center">
                <CountUpStat end={totalMoneyPaid} suffix=" GHS" label="Total Revenue (Paid)" />
                <div className="mt-2 text-sm text-foreground/60">Total Revenue (Paid)</div>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="w-full max-w-6xl">
            ...
            {bookings.length === 0 ? (
              <Card className="bg-card/80 backdrop-blur-sm border-accent/20 w-full">
              ...
              <>
                <Card className="bg-card/80 backdrop-blur-sm border-accent/20 overflow-hidden w-full mx-auto">
                            ...
                            return (
                              <TableRow key={booking.id} className="hover:bg-secondary/60 transition-colors">
                                ...
                                <TableCell className="p-5">
                                  ...
                                  <div className="text-xs text-muted-foreground mt-1">Level {booking.student_level || 'N/A'}</div>
                                </TableCell>
                                <TableCell className="p-5">
                                ...
                                <TableCell className="p-5">
                                  <span className="font-semibold text-accent">GHS {Number(booking.amount).toFixed(2)}</span>
                ...
                {visibleCount < bookings.length && (
                  <div className="mt-12 flex justify-center pb-10">
                    <Button 
                      ...
                      className="bg-accent hover:bg-accent/90 text-accent-foreground px-8 py-6 text-lg rounded-xl shadow-lg shadow-accent/20 transition-all hover:scale-105"
                    >
                      Load More Bookings
                    </Button>
                  </div>
